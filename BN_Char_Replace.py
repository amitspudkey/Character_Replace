...
def main():
    ...
    print()
    print()
    print("This program reads a file and all instances of a character to another.")
    print("It's primary usage is to replace the delimitation of a csv, "
          "assuming all string with that specified character is quoted.")
    print()
    print()
    ...
    # Ask for what character to replace
    replace = input("Character for replacement: ")

    # Ask for what character is the replacement
    replace_with = input("Replacement character: ")

    # Read and write file
    try:
        with io.open(file_in, 'r', encoding=encoder_in) as f:
            text = f.read()
        with io.open(file_out, 'w', encoding=encoder_in) as f:
            f.write(text.replace(replace, replace_with))
    except UnicodeDecodeError:
        print("An error has occurred. Please try again with another input encoder.")
